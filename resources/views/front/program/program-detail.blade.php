        ...
        <div class="container text-center py-5">
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb justify-content-center mb-0">
                    ...
                    <li class="breadcrumb-item text-white" aria-current="page">{{ $program->post_title }}</li>
                </ol>
            </nav>
        </div>
    </div>
        ...
        <div class="container py-5">
            <div class="mx-auto text-center wow fadeIn" data-wow-delay="0.1s" >
                <h4 class="text-primary mb-4 border-bottom border-primary border-2 d-inline-block p-2 title-border-radius">
                    Program Anak</h4>
                <h1 class="mb-5 display-4">{{ $program->post_title }}</h1>
            </div>
            <div class="row g-5 justify-content-center">
                <div class="program-img position-relative">
                    <p style="text-align:center;">by {{ $user }} -
                        {{ \Carbon\Carbon::parse($program->created_at)->translatedFormat('l, d F Y') }}</p>
                    <div class="top-link d-flex gap-3 pe-2" style="justify-content: center; align-items: center; margin-bottom:10px;">
                        ...
                        <!-- Share to Twitter -->
                        <a href="https://twitter.com/intent/tweet?text={{ urlencode($program->post_title) }}&url={{ urlencode(request()->fullUrl()) }}"
                            class="btn btn-secondary btn-sm-square rounded-circle"><i
                                class="fab fa-twitter text-white fa-lg"></i></a>
                        <!-- Share to Whatsapp -->
                        <a href="https://wa.me/?text={{ urlencode($program->post_title . ' ' . request()->fullUrl()) }}"
                            class ="btn btn-secondary btn-sm-square rounded-circle"><i
                ...
                <div class="container-full"
                    ...
                    <h6 style="text-align:center;">{{ $program->post_title }}</h6>
                    @if (is_null($program->notes))
                        <button type="button" class="btn btn-primary">Daftar</button>
                    @else
                        <h6 style="text-align:center; margin-top:10px;"> {{ $program->notes }}</h6>
                    @endif
                </div>
                {{-- <div class="container-full py-5 justify">

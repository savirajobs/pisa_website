...
@section('content')
    ...
    <div class="container-fluid page-header py-5 wow fadeIn" data-wow-delay="0.1s"
        ...
        <div class="container text-center py-5">
            <h1 class="display-2 text-white mb-4">Artikel</h1>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb justify-content-center mb-0">
                    <li class="breadcrumb-item"><a href="#">Beranda</a></li>
                    <li class="breadcrumb-item"><a href="#">Artikel</a></li>
                    <li class="breadcrumb-item text-white" aria-current="page">{{ $article->post_title }}</li>
                </ol>
            </nav>
        </div>
    </div>
    ...
    <div class="container-fluid program py-5">
        <div class="container py-5 ">
            <div class="mx-auto text-center wow fadeIn" data-wow-delay="0.1s">
                ...
                <h1 class="mb-5 display-6">{{ $article->post_title }}</h1>
            </div>
            <div class="row g-5 justify-content-center">
                <div class="program-img position-relative">
                    @if (is_null($article->event_at))
                        <p style="text-align:center;">by {{ $user }} -
                            {{ \Carbon\Carbon::parse($article->created_at)->translatedFormat('l, d F Y') }}</p>
                    @else
                        <p style="text-align:center;">by {{ $user }} -
                            {{ \Carbon\Carbon::parse($article->event_at)->translatedFormat('l, d F Y') }}</p>
                    @endif
                    <div class="top-link d-flex gap-3 pe-2"
                        ...
                        <!-- Share to Twitter -->
                        <a href="https://twitter.com/intent/tweet?text={{ urlencode($article->post_title) }}&url={{ urlencode(request()->fullUrl()) }}"
                            class="btn btn-secondary btn-sm-square rounded-circle"><i
                                class="fab fa-twitter text-white fa-lg"></i></a>
                        <!-- Share to Whatsapp -->
                        <a href="https://wa.me/?text={{ urlencode($article->post_title . ' ' . request()->fullUrl()) }}"
                            class ="btn btn-secondary btn-sm-square rounded-circle"><i
            ...
            <div class="container justify-content-center" style="margin-top:5rem;">
                <h5><span>Artikel Lainnya:</span></h5>
            ...
            <div class="row container-full justify">
                <div class="card-deck d-flex flex-row justify-content-start">
                    @forelse ($relatedarticle as $articles)
                        <div class="col-xl-3 col-md-4 col-sm-6">
                            <div class="card" style="width: 18rem;">
                                <a href="{{ route('frontend.news.show', ['slug' => $articles->slug]) }}">
                                    @if (is_null($articles->image_name))
                                        <img class="card-img-top" src="{{ asset('/asset/img/no-image.jpg') }}"
                                            style="height:215px;">
                                    @else
                                        <img class="card-img-top" src="{{ asset('/images/' . $articles->image_name) }}"
                                            style="height:215px;">
                                    @endif
                                </a>
                                ...
                                <div class="card-body">
                                    <a href="{{ route('frontend.news.show', ['slug' => $articles->slug]) }}">
                                        <h5 class="card-title">{{ $articles->post_title }}</h5>
                                    </a>
                                    <p class="card-text"><small class="text-muted">
                                            @if (is_null($articles->event_at))
                                                {{ \Carbon\Carbon::parse($articles->created_at)->translatedFormat('d F Y') }}
                                            @else
                                                {{ \Carbon\Carbon::parse($articles->event_at)->translatedFormat('d F Y') }}
                                            @endif
                                        </small>
                                    </p>
                                </div>
                    ...
                    @empty
                        <span>Tidak ada Artikel lain.</span>
                    @endforelse
                </div>
            </div>
        </div>
